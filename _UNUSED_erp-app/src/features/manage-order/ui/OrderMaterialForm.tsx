...
import type { OrderConfiguration } from '@entities/order';

interface OrderMaterialFormProps {
  /** Начальные значения формы */
  initialData?: Partial<OrderConfiguration>;
  /** Колбэк при изменении данных */
  onChange?: (data: OrderConfiguration) => void;
}
...
export const OrderMaterialForm = ({ initialData, onChange }: OrderMaterialFormProps) => {
  const [formData, setFormData] = useState<OrderConfiguration>({
  ...
  const handleChange = (field: keyof OrderConfiguration, value: string) => {
