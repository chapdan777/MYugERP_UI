...
import type {
  ...
  UpdateItemInHeaderInput,
  PropertyHeaderProduct,
  AddProductToHeaderInput
} from './types';
...
/**
 * Обновление элемента шапки (сортировка)
 */
export const useUpdateItemInHeader = () => {
  return {
    updateItemInHeader: async (headerId: number, propertyId: number, data: UpdateItemInHeaderInput) => {
      try {
        const response = await apiClient.put<PropertyHeaderItem>(
          `${PROPERTY_HEADERS_API_URL}/${headerId}/items/${propertyId}`,
          data
        );
        // Обновляем кэш SWR
        await mutate(`${PROPERTY_HEADERS_API_URL}/${headerId}/items`);
        ...
        console.error('Ошибка при обновлении элемента шапки:', error);
        throw error;
      }
    },
  };
 ...
 */
export const useGetHeaderProducts = (headerId: number) => {
  const { data, error, isLoading } = useSWR<PropertyHeaderProduct[]>(
    headerId ? `${PROPERTY_HEADERS_API_URL}/${headerId}/products` : null,
    async (url: string) => {
      const response = await apiClient.get(url);
      return response.data;
    }
 ...
 */
export const useAddProductToHeader = () => {
  return {
    addProductToHeader: async (headerId: number, data: AddProductToHeaderInput) => {
      try {
        const response = await apiClient.post<PropertyHeaderProduct>(
          `${PROPERTY_HEADERS_API_URL}/${headerId}/products`,
          data
        );
        // Обновляем кэш SWR
        await mutate(`${PROPERTY_HEADERS_API_URL}/${headerId}/products`);
        return response.data;
      } catch (error) {
        console.error('Ошибка при добавлении продукта в шапку:', error);
        throw error;
      }
    },
  };
 ...
 */
export const useRemoveProductFromHeader = () => {
  return {
    removeProductFromHeader: async (headerId: number, productId: number) => {
      try {
        await apiClient.delete(`${PROPERTY_HEADERS_API_URL}/${headerId}/products/${productId}`);
        // Обновляем кэш SWR
        await mutate(`${PROPERTY_HEADERS_API_URL}/${headerId}/products`);
      } catch (error) {
        console.error('Ошибка при удалении продукта из шапки:', error);
        throw error;
      }
    },
  };
