...
export const useProperties = () => {
  console.log('🔍 useProperties hook called - VERSION 3 (Final Fix)');
  ...
  return {
    updateProperty: async (id: number, propertyData: any) => {
      try {
        ...
        // Note: 'code' can now be updated
        const { dataType, id: _, createdAt, updatedAt, isActive, ...updateData } = propertyData;
        console.log('📤 Sending update property data:', updateData);
        const response = await apiClient.put(`${PROPERTIES_API_URL}/${id}`, updateData);
        return response.data;
      } catch (error) {
